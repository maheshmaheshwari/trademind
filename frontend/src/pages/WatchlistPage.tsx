...
function WatchCard({ item, onRemove, onClick }: { item: WatchlistItem; onRemove: () => void; onClick: () => void }) {
  ...
  const retColor   = item.expReturn >= 0 ? 'var(--green)' : 'var(--red)';
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex flex-col gap-0.5">
          <span className="font-mono text-[18px] font-bold text-[var(--text)]">{inr(item.price)}</span>
          <span className="text-[12.5px] font-semibold tabular-nums" style={{ color: item.change >= 0 ? 'var(--green)' : 'var(--red)' }}>
      ...
      <div className="mb-3">
        <div className="flex justify-between text-[11px] text-[var(--text-3)] mb-1">
          ...
          <span className="font-mono font-bold text-[var(--text)]">{item.confidence}%</span>
        </div>
        <Conf value={item.confidence} />
      </div>
      ...
      <div className="flex justify-between items-center">
        ...
        <span className="font-mono font-bold text-[13px]" style={{ color: retColor }}>
          {item.expReturn >= 0 ? '+' : ''}{item.expReturn.toFixed(2)}%
        </span>
      </div>
        ...
        <div className="mt-3 flex gap-2 flex-wrap">
          {item.alertAbove > 0 && (
            <span className="inline-flex items-center gap-1 h-[20px] px-2 rounded-full text-[10.5px] font-semibold bg-[var(--green-soft)] text-[var(--green)]">
              <Bell size={9} /> ↑ {inr(item.alertAbove)}
            </span>
          )}
          {item.alertBelow > 0 && (
            <span className="inline-flex items-center gap-1 h-[20px] px-2 rounded-full text-[10.5px] font-semibold bg-[var(--red-soft)] text-[var(--red)]">
              <Bell size={9} /> ↓ {inr(item.alertBelow)}
            </span>
          )}
        </div>
      )}
...
export default function WatchlistPage() {
  ...
  const alerts = items.filter(i => i.alertAbove > 0 || i.alertBelow > 0).length;
  const profit = items.filter(i => i.change >= 0).length;
  ...
  return (
    <div className="flex flex-col dgap animate-page-in">
      ...
      {/* ── Stat cards ── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-[calc(16px*var(--u))]">
        {isLoading ? Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} h={90} rounded="14px" />) : (<>
          ...
          <StatCard label="Avg Confidence"  value={items.length ? Math.round(items.reduce((s, i) => s + i.confidence, 0) / items.length) + '%' : '—'} color="var(--accent)" />
      ...
      {!isLoading && items.length > 0 && view === 'grid' && (
        <div className="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-[calc(13px*var(--u))]">
          {sorted.map(item => (
              ...
              <tbody>
                {sorted.map(item => (
                  <tr key={item.symbol} className="cursor-pointer transition-colors hover:bg-[var(--surface-2)]" onClick={() => setDrawer(item.symbol)}>
                    ...
                    <Td align="right" mono>{inr(item.price)}</Td>
                    <Td align="right">
                      <span className="font-mono font-semibold tabular-nums text-[12.5px]" style={{ color: item.change >= 0 ? 'var(--green)' : 'var(--red)' }}>
                        {item.change >= 0 ? '+' : ''}{item.change.toFixed(2)}%
                      </span>
                    </Td>
                    <Td><SignalBadge signal={item.signal} /></Td>
                    <Td><div className="min-w-[110px]"><Conf value={item.confidence} /></div></Td>
                    <Td>
                      <span className="inline-flex items-center h-[22px] px-2 rounded-full text-[11px] font-semibold bg-[var(--surface-3)] text-[var(--text-2)] border border-[var(--border)]">
                        {item.horizon}
                      </span>
                    ...
                    <Td align="right">
                      <span className="font-mono font-semibold tabular-nums text-[12.5px]" style={{ color: item.expReturn >= 0 ? 'var(--green)' : 'var(--red)' }}>
                        {item.expReturn >= 0 ? '+' : ''}{item.expReturn.toFixed(2)}%
                      </span>
                    </Td>
                    <Td>
                      <div className="flex gap-1">
                        {item.alertAbove > 0 && <span className="text-[10px] font-semibold text-[var(--green)] bg-[var(--green-soft)] px-1.5 py-0.5 rounded-full">↑{inr(item.alertAbove)}</span>}
                        {item.alertBelow > 0 && <span className="text-[10px] font-semibold text-[var(--red)] bg-[var(--red-soft)] px-1.5 py-0.5 rounded-full">↓{inr(item.alertBelow)}</span>}
                      </div>
                    </Td>
                    <Td>
                      <button
